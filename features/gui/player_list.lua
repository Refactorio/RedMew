...
local format_number = require '__core__/lualib/util'.format_number
...
local column_builders = {
    ...
    [time_heading_name] = {
        ...
        draw_cell = function(parent, cell_data)
            local text = Utils.format_time_short(cell_data)
    ...
    [coin_heading_name] = {
        ...
        draw_cell = function(parent, cell_data)
            local text = concat({ format_number(cell_data.coin_earned, true), '/', format_number(cell_data.coin_spent, true) })
    ...
    [kills_heading_name] = {
        ...
        draw_cell = function(parent, cell_data)
            local label = parent.add { type = 'label', name = kills_cell_name, caption = format_number(cell_data, true) }
            local label_style = label.style
            label_style.horizontal_align = 'center'
            label_style.width = 70
    ...
    [deaths_heading_name] = {
        ...
        draw_cell = function(parent, cell_data)
            ...
            local label = parent.add { type = 'label', name = deaths_cell_name, caption = cell_data.count, tooltip = tooltip }
            local label_style = label.style
            label_style.horizontal_align = 'center'
            label_style.width = 70
    ...
    [poke_name_heading_name] = {
        ...
        draw_cell = function(parent, cell_data, data)
            ...
            local label = parent.add { type = 'button', name = poke_cell_name, caption = cell_data.poke_count, style = 'crafting_queue_slot' }
            Gui.set_style(label, {
    ...
    [report_heading_name] = {
        ...
        draw_heading = function(parent, sort_symbol)
            ...
            local label_style = label.style
            apply_heading_style(label_style)
            label_style.width = 65

            return label
        end,
        draw_cell = function(parent, cell_data)
            local parent_style = parent.style
            parent_style.width = 65
            parent_style.horizontal_align = 'center'
...
local function draw_main_frame(left, player)
    ...
    Gui.set_style(inner, { padding = 8, right_padding = 0 })

    local heading_table_flow = inner.add { type = 'flow' }

    local cell_table_scroll_pane = inner.add { type = 'scroll-pane' }
    Gui.set_style(cell_table_scroll_pane, { maximal_height = 400 })
...
Gui.on_click(poke_cell_name, function(event)
...
Gui.on_click(report_cell_name, function(event)
