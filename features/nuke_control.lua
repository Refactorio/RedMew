...
local function allowed_to_nuke(player)
    if type(player) == 'table' then
        return player.admin or UserGroups.is_regular(player.name) or
            ((player.online_time / 216000) > global.scenario.config.nuke_control.nuke_min_time_hours)
...
local function on_capsule_used(event)
    ...
    if
        not player or not player.valid or
            (global.scenario.config.nuke_control.enable_autokick and global.scenario.config.nuke_control.enable_autoban)
     then
        return
    end
