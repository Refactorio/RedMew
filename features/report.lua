...
local function draw_report(parent, report_id)
    ...
    local jail_offender_button =
        parent.add {type = 'button', name = jail_offender_button_name, caption = 'Jail ' .. reported_player_name}
...
function Module.jail(target_player, player)
    ...
    if target_player.permission_group == permission_group then
        ...
        target_player.print(
            'You have been placed in jail by ' .. jailed_by .. '. The only action avaliable to you is chatting.'
        )
        ...
        print(
            'Something went wrong in the jailing of ' ..
                target_player.name .. '. You can still change their group via /permissions.'
        )
    end
end
