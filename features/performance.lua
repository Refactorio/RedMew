...
Command.add(
    ...
    function(arguments, player)
        ...
        p({'performance.generic_stat', {'performance.running_speed'}, format('%.2f', stat_mod)})
        p({'performance.generic_stat', {'performance.manual_mining_speed'}, format('%.2f', stat_mod)})
        p({'performance.generic_stat', {'performance.manual_crafting_speed'}, format('%.2f', stat_mod)})
    end
)
...
Command.add(
    ...
    function(_, player)
        ...
        local stat_mod = Performance.get_player_stat_modifier()
        p({'performance.generic_stat', {'performance.game_speed'}, format('%.2f', Performance.get_time_scale())})
        p({'performance.get_formatter', 'performance.game_speed', format('%.2f', stat_mod), 'performance.manual_mining_speed', format('%.2f', stat_mod), 'performance.manual_crafting_speed', format('%.2f', stat_mod)})
    end
)
