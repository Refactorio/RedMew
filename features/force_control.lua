...
local function assert_type(expected_type, given, variable_reference_message)
    ...
    if given_type ~= expected_type then
        error(
            'Argument ' ..
                variable_reference_message .. " must be of type '" .. expected_type .. "', given '" .. given_type .. "'"
        )
    end
end
...
function ForceControl.get_force_control_builder()
    if not next_level_cap_calculator.execute then
        error(
            'Can only get the force control builder when the config has been initialized via ForceControl.register(config_table).'
        )
    end
