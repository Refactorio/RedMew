...
local function redraw_table(data)
    ...
    for _, data in pairs(score_keys) do
        ...
        local val =
            table.add({type = 'label', name = 'Diggy.ArtefactHunting.Frame.List.Val', caption = utils.comma_value(val)})
    ...
    Event.add(
        ...
        function(event)
            ...
            if coins > 0 then
                ...
                game.print(
                    'sent ' .. coins .. ' coins into space! The space station is now holding ' .. sum .. ' coins.'
                )
            end
        end
    )
            ...
            function(event)
                ...
                for x = area.left_top.x, area.left_top.x + 31 do
                    ...
                    for y = area.left_top.y, area.left_top.y + 31 do
                        if
                            sq_x + y * y >= distance_required and
                                get_noise(surface, x, y) >= config.treasure_chest_noise_threshold
                         then
