...
function ScatteredResources.register(config)
    ...
    local function spawn_cluster_resource(surface, x, y, cluster_index, cluster)
        ...
        local range =
            resource_richness_values[get_name_by_weight(resource_richness_weights, resource_richness_weights_sum)]
                ...
                for index, cluster in ipairs(c_clusters) do
                    if distance >= cluster.min_distance and cluster.noise_settings.type ~= 'skip' then
                        if cluster.noise_settings.type == 'connected_tendril' then
                            ...
                            if
                                -1 * cluster.noise_settings.threshold < noise and
                                    noise < cluster.noise_settings.threshold
                             then
                        ...
                        elseif cluster.noise_settings.type == 'fragmented_tendril' then
                            ...
                            if
                                -1 * cluster.noise_settings.threshold < noise1 and
                                    noise1 < cluster.noise_settings.threshold and
            ...
            if s_mode then
                ...
                if (probability > random()) then
                    ...
                    local range =
                        ...
                        get_name_by_weight(resource_richness_weights, resource_richness_weights_sum)
                    ]
                    local amount = random(range[1], range[2])
                    ...
                    Template.resources(
                        surface,
                        {{name = resource_name, position = {x = x, y = y}, amount = ceil(amount)}}
                    )
                end
            end
        end
                        ...
                        for index, cluster in ipairs(c_clusters) do
                            if cluster.noise_settings.type == 'connected_tendril' then
                                ...
                                if
                                 ...
                                 then
                                    color[index] =
                                        color[index] or cluster.color or {r = random(), g = random(), b = random()}
                                    Debug.print_colored_grid_value(
                                        'o' .. index,
                                        surface,
                                        {x = x, y = y},
                                        nil,
