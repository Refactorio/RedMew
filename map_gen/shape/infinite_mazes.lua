...
local function get_maze(x, y, seed, width, height)
    ...
    if not global.maze_data then
        global.maze_data = {}
    end
    if not global.maze_data[x] then
        global.maze_data[x] = {}
    end
...
local function get_maze_ore(x, y, seed, width, height)
    ...
    if not global.maze_data_ore then
        global.maze_data_ore = {}
    end
    if not global.maze_data_ore[x] then
        global.maze_data_ore[x] = {}
    end
...
function run_shape_module(event)
    if not global.maze_seed then
        global.maze_seed = math.random(0, 65536 * 65536 - 1)
    end
