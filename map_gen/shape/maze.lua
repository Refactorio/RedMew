...
function init()
    if not global.walk_seed_w then
        global.walk_seed_w = math.random(1, 50) * 2
    end
    if not global.rects then
        global.rects = {}
    end
    if not global.walk_seed_h then
        global.walk_seed_h = math.random(1, 50) * 2
    end
...
function run_shape_module(event)
    ...
    if
        math.abs(pos.x) > 10000 or math.abs(pos.y) > 10000 or
            (global.rects[pos.x + global.max / 2 .. '/' .. pos.y + global.max / 2] == nil)
     then
        removeChunk(event)
    end
end
