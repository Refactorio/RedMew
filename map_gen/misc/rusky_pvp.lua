...
Event.add(
    ...
    function(event)
        ...
        player.force.chart(
            player.surface,
            {{player.position.x - 200, player.position.y - 200}, {player.position.x + 200, player.position.y + 200}}
        )
    end
)
            ...
            for index, player in pairs(force.players) do
                if player.gui.left.rocket_score == nil then
                    ...
                    frame.add {
                        name = 'rocket_count',
                        type = 'label',
                        caption = tostring(global.satellite_sent[force.name])
                    }
                else
...
function killBitters(pos)
    for k, v in pairs(
        game.surfaces[1].find_entities_filtered(
            {area = {{pos.x - 250, pos.y - 250}, {pos.x + 250, pos.y + 250}}, force = 'enemy'}
        )
    ) do
        v.destroy()
    end
...
function validPlayer(name)
    if
        name ~= nil and Game.get_player_by_index(name) ~= nil and
            Game.get_player_by_index(name).force == game.forces.player
     then
        return true
    end
    return false
