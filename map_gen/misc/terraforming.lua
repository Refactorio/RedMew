...
local function convert_tiles(tile_table, tiles)
    local set_tiles = game.surfaces.redmew.set_tiles
    local tile_set = {}
...
local function check_chunk_for_entities(chunk)
    local find_entities_filtered = game.surfaces.redmew.find_entities_filtered
...
local function change_creep_state(state, i)
    local find_tiles_filtered = game.surfaces.redmew.find_tiles_filtered
    local tiles_to_set = {}
    local debug_message
    ...
    if (#tiles > 0) then
        ...
        if state == 2 then
            ...
            game.surfaces.redmew.regenerate_decorative(decoratives, {{chunklist[i].x, chunklist[i].y}})
        end
    end
end
