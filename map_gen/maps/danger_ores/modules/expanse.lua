...
return function(config)
    ...
    local TILE_VECTORS = { { -RADIUS, 0 }, { RADIUS, 0 }, { 0, RADIUS }, { 0, -RADIUS } }
    local START_RADIUS = (start_size / 2)
    local function is_container_position_valid(position)
        local x = math_floor(position.x)
        local y = math_floor(position.y)
        local abs_x = math_abs(x) + ((x > 0) and 1 or 0)
        local abs_y = math_abs(y) + ((y > 0) and 1 or 0)

        if abs_x == START_RADIUS then
            if abs_y <= START_RADIUS then
                return true
            end
        end
        if abs_y == START_RADIUS then
        ...
        for _, v in pairs(TILE_VECTORS) do
            local tile = surface.get_tile({ x + v[1], y + v[2] })
    ...
    local function create_costs_render(entity, name, offset)
        local obj1 = rendering.draw_sprite {
            ...
            y_scale = 1.1,
            render_layer = '190',
            target = { entity = entity, offset = { x = offset, y = -1.5 } },
    ...
    local function init_chest(entity, budget)
        ...
        for k, v in pairs(item_stacks) do
            table.insert(price, { name = k, count = v, type = 'item', render = create_costs_render(entity, k, offset) })
            offset = offset + 1
        end
    ...
    local function process_chest(entity, budget)
        ...
        if game.tick > 300 and not is_container_position_valid(entity.position) then
    ...
    local function on_chunk_unlocked(event)
        ...
        local old_chests = surface.find_entities_filtered {
        ...
        if game.tick > 60 then
            game.print({ 'danger_ores.chest_fulfilled', math.floor(left_top.x + 16), math.floor(left_top.y + 16), RS.get_surface_name() })
        end
    ...
    local function on_tick()
        ...
        if not (chest and chest.entity and chest.entity.valid) then
            primitives.index = nil
            if idx then
                chest_data[idx] = nil
            end
    ...
    local function infinite_resource(event)
        ...
        elseif entity.type == 'simple-entity' then
            ...
            surface.spill_item_stack {
                position = entity.position,
                stack = { name = ROCK_RESOURCES[math.random(#ROCK_RESOURCES)], count = math.random(80, 160) },
                enable_looted = true,
                allow_belts = true,
            }
    ...
    local initial_setup = Token.register(function()
        local rs = RS.get_surface()
        for _, e in pairs(surface.find_entities_filtered{ type = { 'simple-entity', 'tree' }, radius = 15, position = { x = 0, y = 0}}) do
            e.destroy()
        end
        ...
        oil.amount, oil.initial_amount = 3e6, 3e7
    ...
    Global.register_init({ chest_data = chest_data, primitives = primitives, grid = grid }, function(tbl)
        local rs = RS.get_surface()
        ...
        Task.set_timeout_in_ticks(30, initial_setup)
        tbl.surface = rs
    end, function(tbl)
        surface = tbl.surface
