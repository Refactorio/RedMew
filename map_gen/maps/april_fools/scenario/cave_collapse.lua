--[[
  Refactored version from Diggy/diggy_cave_collapse.lua
...
local collapse_rocks_map = Template.diggy_rocks_map
...
local config = {
  enabled = true,
  -- shows players an initial warning about mining stones
  onboarding = false,
  ...
  collapse_threshold_total_strength = 40, --16
...
for ___, rock_name in pairs(collapse_rocks) do
  if rock_name:find('big', 1, true) ~= nil then
    config.support_beam_entities[rock_name] = 2
  elseif rock_name:find('huge', 1, true) ~= nil then
    config.support_beam_entities[rock_name] = 2.5
  end
end
...
local function collapse(args)
  ...
  raise_event(CaveCollapse.events.on_collapse, args)
...
local function on_collapse_triggered(event)
  ...
  if x_t and x_t[y] then
    template_insert(surface, {}, { { position = position, name = collapse_rocks[math.random(collapse_rocks_size)] } })
    return
  end
...
local function add_fraction(stress_map, x, y, fraction, player_index, surface)
  ...
  if fraction > 0 then
    if value > stress_threshold_causing_collapse then
      raise_event(CaveCollapse.events.on_collapse_triggered, { surface = surface, position = { x = x, y = y }, player_index = player_index })
...
stress_map_add = function(surface, position, factor, no_blur, player_index)
  ...
  if not stress_map then
    game.print("error - no_stress_map")
    return
  end
...
CaveCollapse.stress_map_add = stress_map_add
...
-- General events
Event.add(CaveCollapse.events.on_collapse_triggered, on_collapse_triggered)
Event.add(Template.events.on_void_removed, on_void_removed)
-- Not raised because 'mines' is generated on_init
Event.add(defines.events.on_surface_created, on_surface_created)

if config.onboarding then
