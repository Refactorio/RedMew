...
Event.add(defines.events.on_chunk_generated, function(event)
  ...
  for x = tx, bx do
    for y = ty, by do
      if not inside_radius(x, y, starting_radius) and math.random() > 0.55 then
        surface.create_entity { name = rock_list[math.random(rocks)], position = { x, y }, raise_built = false }
      end
    end
  end
end)
...
Event.add(defines.events.on_player_mined_entity, function(event)
  ...
  if math.random() < 0.08 and not inside_radius(pos.x, pos.y, biter_radius) then
    entity.surface.create_entity {
      name = math.random() > 0.5 and 'biter-spawner' or 'spitter-spawner',
