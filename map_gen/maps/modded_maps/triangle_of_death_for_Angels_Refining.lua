--[[
This the Triangle of Death map script adapted for Angels Refining.
To account for the increased difficulty from the longer ore refining chains the usual -50% player damage malus has been disabled.
Loot generation has also been disabled.
This map script does NOT include any changes to account for Bob's Enemies or Bob's Warfare.
If you want to use those you need to change biter and loot generation.
]]
...
-- disabling the normal ore generation since we do our own one later
...
-- for the sake of balance around the more complicated start we disable the player damage reduction from the vanilla script
--[[
Event.add(
...
Global.register_init(
    ...
    function(tbl)
        ...
        --init_weapon_damage() -- disabling the player damage reduction
    end,
    function(tbl)
...
-- making the water border a bit wider to account for the seafloor pump
local sea = b.rectangle(3,3)
sea = b.translate(map, 0, -8)
...
local ores = {
        {transform = non_transform, resource = 'angels-ore1', value = value(1400, 0.1), weight = 16},   --saphirite (dark blue)
        {transform = non_transform, resource = 'angels-ore2', value = value(1000, 0.1), weight = 16},   --jivolite (yellow)
        {transform = non_transform, resource = 'angels-ore3', value = value(1200, 0.1), weight = 16},   --stiratite (light blue)
        {transform = non_transform, resource = 'angels-ore4', value = value(1000, 0.1), weight = 16},   --crotinium (white)
        {transform = non_transform, resource = 'angels-ore5', value = value(1000, 0.1), weight = 16},   --rubyte (red)
        {transform = non_transform, resource = 'angels-ore6', value = value(1000, 0.1), weight = 16},   --bobmonium (brown)
        {transform = non_transform, resource = 'coal', value = value(1200, 0.1), weight = 16},
        {transform = oil_transform, resource = 'crude-oil', value = value(1000000, 50), weight = 12},
        {transform = oil_transform, resource = 'angels-natural-gas', value = value(10000, 50), weight = 12},
        {transform = oil_transform, resource = 'angels-fissure', value = value(10000, 50), weight = 10},
        {transform = empty_transform, weight = 200}
    }
...
local start_saphirite = b.resource(start_ore, 'angels-ore1', value(1200, 0))
local start_stiratite = b.resource(start_ore, 'angels-ore3', value(1200, 0))
local start_coal = b.resource(start_ore, 'coal', value(2000, 0))
start_ore = b.segment_pattern({start_coal,start_stiratite , start_saphirite})
