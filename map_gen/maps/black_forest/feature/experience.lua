...
function Experience.update_market_contents(force)
    ...
    for _, prototype in pairs(config.unlockables) do
        ...
        if current_level < prototype_level then
            disable_item(force_name, prototype.name, {'black_forest.market_disabled', prototype_level})
        else
...
local function on_player_mined_entity(event)
    ...
    if name == 'tree-01' then
        exp = tree_01_xp + floor(level / 5)
    elseif name == 'tree-02' then
        exp = tree_02_xp + floor(level / 5)
    elseif name == 'tree-03' then
        exp = tree_03_xp + floor(level / 5)
    end
    ...
    local text = {'', '[img=entity/' .. name .. '] ', {'black_forest.float_xp_gained_mine', exp}}
...
local function on_research_finished(event)
    ...
    local text = {'', '[img=item/automation-science-pack] ', {'black_forest.float_xp_gained_research', exp}}
...
local function on_rocket_launched(event)
    ...
    local text = {'', '[img=item/satellite] ', {'black_forest.float_xp_gained_rocket', exp}}
    for _, p in pairs(game.connected_players) do
        local player_index = p.index
        print_player_floating_text_position(player_index, text, gain_xp_color, -1, -0.5)
    end
...
local function on_entity_died(event)
    ...
    if not cause or not cause.valid or cause.type ~= 'character' then
        ...
        if force and force.name == 'player' then
            if cause and (cause.name == 'artillery-turret' or cause.name == 'gun-turret' or cause.name == 'laser-turret' or cause.name == 'flamethrower-turret') then
                ...
                if entity_name == 'tree-01' then
                    exp = floor((tree_01_xp + level * 0.2) * 0.5)
                elseif entity_name == 'tree-02' then
                    exp = floor((tree_02_xp + level * 0.2) * 0.5)
                elseif entity_name == 'tree-03' then
                    exp = floor((tree_03_xp + level * 0.2) * 0.5)
                end
        ...
        if exp > 0 then
            Game.print_floating_text(entity.surface, floating_text_position, {'', '[img=entity/' .. entity_name .. '] ', {'black_forest.float_xp_gained_kill', exp}}, gain_xp_color)
            add_experience(force, exp)
        end
    ...
    print_player_floating_text_position(cause.player.index, {'', '[img=entity/' .. entity_name .. '] ', {'black_forest.float_xp_gained_kill', exp}}, gain_xp_color, -1, -0.5)
    add_experience(force, exp)
end
...
local function on_player_respawned(event)
    ...
    local text = {'', '[img=entity.character]', {'black_forest.float_xp_drain', exp}}
    game.print({'black_forest.player_drained_xp', player.name, exp}, lose_xp_color)
    for _, p in pairs(game.connected_players) do
...
local function redraw_title(data)
    ...
    data.frame.caption = {'black_forest.gui_total_xp', Utils.comma_value(force_data.total_experience)}
end
...
local function redraw_heading(data, header)
    ...
    local header_caption = (head_condition) and {'black_forest.gui_reward_item'} or {'black_forest.gui_reward_buff'}
    Gui.clear(frame)

    local heading_table = frame.add(table_column_layout)
    apply_heading_style(heading_table.add({type = 'label', caption = {'black_forest.gui_requirement'}}).style, 100)
...
local function redraw_progressbar(data)
    ...
    apply_heading_style(
        flow.add(
            {
                type = 'label',
                tooltip = {'black_forest.gui_progress_tip', force_data.current_level, Utils.comma_value((force_data.total_experience - force_data.current_experience) + force_data.experience_level_up_cap), Utils.comma_value(force_data.experience_level_up_cap - force_data.current_experience)},
                name = 'black_forest.Experience.Frame.Progress.Level',
                caption = {'black_forest.gui_progress_caption'}
            }
        ).style
    )
    local level_progressbar = flow.add({type = 'progressbar', tooltip = {'black_forest.gui_progress_bar', floor(force_data.experience_percentage * 100) * 0.01}})
...
local function redraw_table(data)
    ...
    for _, prototype in pairs(config.unlockables) do
        ...
        if first_item_for_level then
            level_caption = {'black_forest.gui_tabel_level', current_item_level}
        end
        ...
        local level_column =
            list.add(
            {
                ...
                tooltip = {'black_forest.gui_tabel_xp', Utils.comma_value(calculate_level_xp(current_item_level))}
            }
        )
...
local function redraw_buff(data)
    ...
    for name, effects in pairs(config.buffs) do
        ...
        if not all_levels_shown then
            ...
            level_caption = {'black_forest.gui_buff_level'}
        end
        ...
        if name == 'mining_speed' then
            buff_caption = {'black_forest.gui_buff_mining', effect_value, effect_max * 100}
        elseif name == 'inventory_slot' then
            buff_caption = {'black_forest.gui_buff_inv', effect_value, effect_max}
        elseif name == 'health_bonus' then
            buff_caption = {'black_forest.gui_buff_health', effect_value, effect_max}
        else
            buff_caption = {'black_forest.gui_buff_other', effect_value, name}
        end
...
local function toggle(event)
    ...
    local frame = left['black_forest.Experience.Frame']
    ...
    frame = left.add({name = 'black_forest.Experience.Frame', type = 'frame', direction = 'vertical'})
    ...
    frame.add({type = 'button', name = 'black_forest.Experience.Button', caption = {'black_forest.gui_close_btn'}})
...
local function on_player_created(event)
    game.get_player(event.player_index).gui.top.add(
        {
            name = 'black_forest.Experience.Button',
            type = 'sprite-button',
            sprite = 'entity/market',
            tooltip = {'black_forest.gui_experience_button_tip'}
        }
    )
end

Gui.allow_player_to_toggle_top_element_visibility('black_forest.Experience.Button')

Gui.on_click('black_forest.Experience.Button', toggle)
Gui.on_custom_close(
    'black_forest.Experience.Frame',
    function(event)
...
local function update_gui()
    ...
    for i = #players, 1, -1 do
        local p = players[i]
        local frame = p.gui.left['black_forest.Experience.Frame']
...
function Experience.register(cfg)
    ScoreTracker.register(experience_lost_name, {'black_forest.score_experience_lost'}, '[img=recipe.artillery-targeting-remote]')
    ...
    ForceControlBuilder.register_on_every_level(
        function(level_reached, force)
            Toast.toast_force(force, 10, {'black_forest.toast_new_level', level_reached})
    ...
    tree_01_xp = config.XP['tree-01']
    tree_02_xp = config.XP['tree-02']
    tree_03_xp = config.XP['tree-03']
end
