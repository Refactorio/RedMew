...
--local black_forestCaveCollapse = require 'map_gen.maps.black_forest.feature.black_forest_cave_collapse'
...
function StartingZone.register(config)
    ...
    local function on_chunk_generated(event)
        ...
        -- hack to figure out whether the important chunks are generated via black_forest.feature.refresh_map.
            ...
            for y = -starting_zone_size, starting_zone_size do
                ...
                if (distance < starting_zone_size) then
                    ...
                    if (distance > rock_range) then
                        insert(rocks, {name = 'tree-01', position = {x = x+0.5, y = y+0.5}})
                    end

                    -- hack to avoid starting area from collapsing
                    --if (distance > stress_hack) then
                        --black_forestCaveCollapse.stress_map_add(surface, {x = x, y = y}, -0.5)
                    --end
                end
            end
        end
        ...
        Retailer.set_market_group_label('player', 'Black Forest Market')
